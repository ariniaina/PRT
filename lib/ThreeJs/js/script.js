...
function init() {
	...
	function initControls() {
		...
		for (var i=0;i<opts.length;i++) {
			opts[i].onclick = register.doNewRegistration;	
		}
...
var register = {
	events: [],		// filled according to event checkboxes
	elements: [],		// filled according to element checkboxes
	currentRegistrationMode: undefined,
	Traditional: function (flag) {
		var newValue = (flag) ? handleEvents : null;
		for (var i=0;i<this.elements.length;i++) {
			for (var j=0;j<this.events.length;j++) {
				this.elements[i]['on'+this.events[j].toLowerCase()] = newValue;
			}
		}	
	},
	...
	doNewRegistration: function() {		// may be called as event handler
		var opts = $('controlForm').elements['registration'];
		for (var i=0;i<opts.length;i++) {
			if (opts[i].checked) {
...
function handleEvents(e,elName) {
	...
	var forbiddenElements = [$('controlForm'),$('logContainer')];
	
	/* Turn off this script block to see the events that take place
	in the log and control panels, too. */
	if (tgt && tgt !== window) {
	...
	log.msg(writestring);
	if (tgt && tgt.getAttribute && tgt.getAttribute('fnCall')) {
		window[tgt.getAttribute('fnCall')](evt.type);
	}
	...
	if ($('preventDefault').checked) {
		if (evt.preventManipulation) {
			evt.preventManipulation();	// MS pointer events
			alert('jep');
		}
...
function loadAssets() {
	var radios = $('assetForm').elements;
	var selectedAsset;
	for (var i=0;i<radios.length;i+=1) {
		if (radios[i].checked) {
			selectedAsset = radios[i].value;
			break;
		}
	}
	var tgt;
	if (selectedAsset) {
		tgt = $(ids[selectedAsset]);	
		var src = desiredSources[selectedAsset];
		if (selectedAsset === 'link') {
			tgt.setAttribute('href',src);
		}
		else {
			tgt.src = src;
		}
		var tmp = alternateSources[selectedAsset];
		alternateSources[selectedAsset] = src;
		desiredSources[selectedAsset] = tmp;
	}
	return false;
}
